...
def analise_padroes_sequencias_lotofacil(dados_sorteios):
    """
    Análise completa de padrões e sequências dos números da Lotofácil (1-25, 15 dezenas)
    
    Args:
        dados_sorteios (list): Lista de listas com os sorteios
        Formato: [[concurso, bola1, ..., bola15], ...]
    ...
    for sorteio in dados_sorteios:
        if len(sorteio) >= 16:  # Concurso + 15 números
            concurso_val = int(sorteio[0])
            numeros_vals = [int(n) for n in sorteio[1:16]]
            historico_sorteios.append({
                'concurso': concurso_val,
                'numeros': sorted(numeros_vals),
                'numeros_originais': numeros_vals
            })
    ...
    def analisar_consecutivos():
        consecutivos_stats = {
            ...
            # Novo: mapeia tamanho -> lista de concursos que tiveram pelo menos um bloco desse tamanho
            'consecutivos_por_tamanho_concursos': {}
        }
        ...
        for sorteio in historico_sorteios:
            ...
            if consecutivos_no_sorteio:
                ...
                for seq in consecutivos_no_sorteio:
                    ...
                    consecutivos_stats['consecutivos_por_tamanho_concursos'].setdefault(len(seq), []).append(sorteio['concurso'])
        
        # Campos de resumo esperados pelo frontend
        total_concursos = len(historico_sorteios)
        total_com = len(consecutivos_stats['por_concurso'])
        percentual = (total_com / total_concursos * 100) if total_concursos else 0.0

        consecutivos_stats['total_concursos_com_consecutivos'] = int(total_com)
        consecutivos_stats['percentual_concursos_com_consecutivos'] = round(float(percentual), 2)
        consecutivos_stats['maior_sequencia_consecutiva'] = int(consecutivos_stats['maior_sequencia'])
    ...
    def analisar_intervalos():
        ...
        # Inicializar contadores para cada número (1-25 para Lotofácil)
    ...
    # Conversão segura para tipos nativos JSON-serializáveis
    def _to_native(obj):
        import numpy as _np
...
def analise_padroes_sequencias_lotofacil_completa(df_lotofacil, qtd_concursos=None):
    """
    ...
    colunas_necessarias = ['Concurso'] + [f'Bola{i}' for i in range(1, 16)]
    colunas_faltantes = [col for col in colunas_necessarias if col not in df_lotofacil.columns]
    ...
    for _, row in df_lotofacil.iterrows():
        # Verificar se os dados são válidos (apenas números 1-25)
        if pd.isna(row['Concurso']) or any(pd.isna(row[col]) for col in [f'Bola{i}' for i in range(1, 16)]):
            continue  # Pular linhas com dados inválidos
        
        # Validar range de números (1-25 para Lotofácil)
        numeros_validos = [row[f'Bola{i}'] for i in range(1, 16)]
        if all(1 <= n <= 25 for n in numeros_validos):
            sorteio = [row['Concurso']] + numeros_validos
    ...
    resultado = analise_padroes_sequencias_lotofacil(dados_sorteios)

    return resultado

def analisar_padroes_sequencias_lotofacil(df_lotofacil=None, qtd_concursos=50):
    """
    Wrapper para análise de padrões/seqüências da Lotofácil.
    Carrega dados se necessário e filtra últimos N concursos antes de chamar a análise completa.
    """
    try:
        ...
        resultado_completo = analise_padroes_sequencias_lotofacil_completa(df_filtrado, qtd_concursos=None)
