...
# Função para redefinir a data de instalação (somente para testes)
def reset_installation_date():
    install_date = datetime.datetime.now()
    with open(installation_date_file, 'w') as file:
        file.write(install_date.strftime('%Y-%m-%d'))

# Redefinir a data de instalação (somente para testes)
...
# Função para rodar o loop principal do Tkinter
def run_tk():
    main_root = tk.Tk()
